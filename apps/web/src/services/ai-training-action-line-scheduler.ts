...
export type AiTrainingActionLineScheduleSummary = {
  programs: number;
  candidates: number;
  broadcasts: number;
  recipients: number;
...
export async function scheduleAiTrainingActionLineDeliveries(input: {
  ...
  now?: Date;
  limit?: number;
}): Promise<AiTrainingActionLineScheduleSummary> {
  const db = await import('@bunshin/database');
  const now = input.now ?? new Date();
  const limit = Math.min(Math.max(input.limit ?? 500, 1), 500);
  const summary: AiTrainingActionLineScheduleSummary = {
    programs: 0,
    candidates: 0,
    broadcasts: 0,
    recipients: 0,
  ...
  const runtime = new AiTrainingParticipantService(
    ...
    try {
      ...
      >`
        SELECT
          ...
          membership."user_id" AS "participantUserId"
        ...
        INNER JOIN "training_participant_profiles" profile
          ON profile."program_enrollment_id" = enrollment."id"
          AND profile."workspace_id" = enrollment."workspace_id"
          AND profile."group_id" = enrollment."group_id"
        ...
        ORDER BY enrollment."created_at" ASC, enrollment."id" ASC
        LIMIT ${remaining + 1}
      `;
      const runtimeResults = await Promise.allSettled(
        runtimeCandidates.slice(0, remaining).map((candidate) =>
          runtime.current({
            workspaceId: program.workspaceId,
            groupId: program.groupId,
            actorUserId: candidate.participantUserId,
      ...
      for (const assignment of assignments) {
        ...
        const display = parseAiTrainingActionDisplay(assignment.displaySnapshot);
        ...
        const message = buildAiTrainingActionLineMessage({
