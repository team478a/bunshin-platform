import 'server-only';
import { RequestVideoRenderRetry } from '@bunshin/application';
...
export async function retryVideoRenderResponse(request: Request, renderId: string) {
  ...
  try {
    ...
    const value = await new RequestVideoRenderRetry(
