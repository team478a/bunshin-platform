...
import { loadServiceBunshinDailyActions } from './service-bunshin-detail-daily-actions';
import { resolveServiceBunshinBusinessDetail } from './service-bunshin-detail-business';
...
export async function loadServiceBunshinDetail({
  ...
  const { dedicatedLine, dedicatedLineConnection } = await loadServiceDedicatedLineConnection({
    db,
    workspaceId: service.workspaceId,
    groupId: service.serviceId,
    actorUserId: actor.userId,
  });
  const dailyActions = await loadServiceBunshinDailyActions({
    db,
    workspaceId: service.workspaceId,
    groupId: service.serviceId,
    bunshinId,
  ...
  const socialInsightSnapshots = await loadServiceBunshinSocialInsights({
    db,
  ...
  const { approvedBusinessStrategy, successfulBusinessTopic, businessProgram } =
    resolveServiceBunshinBusinessDetail({
      enabled: isBusinessDailyService,
      accountStrategies,
      dailyMissions,
      programStartedAt: businessProgramStartedAt,
      currentDate: today,
    });
  return {
    actor,
