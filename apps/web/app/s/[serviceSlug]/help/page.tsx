...
export default async function ServiceHelpPage({
  ...
  const rewardsPilotAccess =
    !isBusinessDailyService && user && membership
    ...
    <PublicShell showPlatformBrand={false}>
      <article className="service-entry service-help" style={style}>
        ...
        <nav className="service-help__nav" aria-label="ヘルプ内メニュー">
          ...
          {!isBusinessDailyService && rewardsPilotAccess && <a href="#rewards">ポイント・バッジ</a>}
          {!isBusinessDailyService && <a href="#media">画像・動画</a>}
        ...
        <section className="service-entry__card service-help__section" id="start">
          ...
          {isBusinessDailyService ? (
            <ol className="service-help__steps">
              <li>LINEでログインし、サービスへの参加を完了します。</li>
              <li>業種、商品・サービス、伝えたい相手などを入力します。</li>
              <li>投稿を一緒に作る「投稿パートナー」を設定します。</li>
              <li>毎日LINEに届く案内から、完成した投稿文を確認します。</li>
            </ol>
          ) : (
            <ol className="service-help__steps">
        ...
        <section className="service-entry__card service-help__section" id="daily">
          ...
          <ol className="service-help__steps">
            ...
            <li>
              {isBusinessDailyService
                ? '採用した投稿文をコピーし、必要なところだけ直します。'
                : '採用した投稿文や指示文をコピーし、必要に応じて整えます。'}
            </li>
            <li>ご自身のSNSへ貼り付けて投稿します。SNSへの自動公開は行われません。</li>
            <li>
              投稿後に「投稿しました」を押します。この記録は自己申告で、SNSへの投稿は自動確認されません。
            </li>
          </ol>
          {!isBusinessDailyService && (
        ...
        {!isBusinessDailyService && rewardsPilotAccess && (
          <section className="service-entry__card service-help__section" id="rewards">
            ...
            <ol className="service-help__steps">
              <li>
                最新の投稿案を開きます。その日に初めて開いた投稿案がポイントの対象になります。
              </li>
          ...
          <div className="service-help__faq">
            <details>
              ...
              <div>
                <p>
                  {isBusinessDailyService
                    ? 'LINEの友だち状態と通知設定を確認してください。'
                    : 'まず、その日が週間予定の投稿日か確認してください。予定がない日は通常配信されません。'}
                </p>
                <p>
                  {isBusinessDailyService
                    ? '問題が見つからない場合は、サービスの参加状態を確認し、運営者へ連絡してください。'
            ...
            <details>
              ...
              <div>
                <p>
                  {isBusinessDailyService
                    ? '投稿文を準備している可能性があります。少し待って画面を開き直し、表示されない場合は運営者へ連絡してください。'
        ...
        {(isManager || isContentEditor) && (
          <section className="service-entry__card service-help__section" id="operation">
            ...
            <ol className="service-help__steps">
              ...
              <li>
                {isBusinessDailyService
                  ? '公式資料、FAQ、必須表記、避ける表現を登録します。'
                  : '公式資料、FAQ、商品情報、必須表記、避ける表現を登録します。'}
              </li>
              <li>テスト参加者1名で、登録から投稿案の確認、LINE通知まで試します。</li>
              <li>
                {isBusinessDailyService
                  ? '開始後は、投稿文の準備状況とLINEの配信失敗を毎日確認します。'
                  : '開始後は配信失敗、生成失敗、承認待ち、利用枠を毎日確認します。'}
              </li>
            </ol>
            <div className="service-home-actions">
              {isManager && (
