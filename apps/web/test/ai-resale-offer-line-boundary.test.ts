...
describe('AI resale DAY7 offer LINE and funnel boundary', () => {
  ...
  it('revalidates the current offer immediately before LINE delivery', () => {
    expect(eligibility).toContain("criteria.kind === 'AI_RESALE_OFFER'");
    expect(eligibility).toContain("state?.status !== 'STANDARD'");
    expect(eligibility).toContain('state.offer?.offeringId !== criteria.offeringId');
    expect(eligibility).toContain("eventType: 'STANDARD_OFFER_SHOWN'");
    expect(eligibility).toContain('alreadyShown');
  });
