...
外部処理中は指数Backoffで再試行し、上限到達時はRenderとProjectを失敗状態へ揃える。課金、SNS自動投稿、Webhookのみを信用した完了判定は実装していない。

## 保存期限と自動削除

利用者がアップロードした動画素材、AI場面、最終動画、SNS画像の元画像・完成画像・サムネイルは、作成またはアップロード完了から90日間だけ非公開Storageに保持する。保存期限の7日前からサービス管理者の動画運用画面へ件数を表示し、利用者自身で必要な完成物を保存できるようにする。

ProductionのCronだけが毎日2:30（日本時間）に期限切れのobjectを削除する。削除APIはCron Secretを必須とし、開発・Preview・Stagingでは実削除しない。Storage削除が成功した後だけDBを`DELETED`または削除日時へ更新するため、失敗時は次回のCronで安全に再試行できる。プロンプト、署名URL、外部Job IDは削除ログや管理画面へ出さない。

アップロード時の実形式・容量検査は既存の完了処理で実施する。外部マルウェア検査Providerの接続は、検査対象・保持期間・費用・誤検知時の運用を別途決定してから追加する。
...
## V-5B3B 管理者向け運用監視と安全な再実行

システム管理画面に、現在の実行環境に属する動画生成だけを一覧表示する運用画面を追加した。処理待ち、送信済み、作成中、完成、失敗、取消の件数と直近100件を確認できる。台本本文、外部完成URL、APIキーは表示しない。

手動再実行は`SUPER_ADMIN`または`OPERATOR`だけが、再実行可能と定義した一時障害に対して、3文字以上の理由を入力した場合に実行できる。認証失敗、設定不正、権限・所有境界の不一致、停止済みGroup等は再実行しない。外部Job IDがある場合は新規Renderを送信せず状態確認から再開し、外部Job IDがない場合だけ送信待ちへ戻す。

同じ失敗発生時刻に対する再実行要求はDB一意制約で1回に限定する。要求者、対象環境、理由、対象Render、起動Job、日時を専用履歴へ保存し、元のRender履歴を上書きしない。再実行時にもWorkspace、Group、User、Membership、動画機能権限と実行環境をサーバー側で検証する。

完成本数の確定と完成通知はV-5B3Cで扱う。

## V-5B3C 完成時の利用回数確定と本人通知

Private Storageへの保存まで成功し、Renderが`SUCCEEDED`になった動画だけを利用回数へ記録する。`video-render-completed:{renderId}`を操作キーとして既存のGroup機能利用履歴へ保存するため、Polling、Webhook、Job再試行が重なっても同じ動画を二重計上しない。失敗、取消、作成途中は計上しない。
