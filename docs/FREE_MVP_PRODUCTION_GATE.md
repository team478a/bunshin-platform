...
## H. 外部成果計測URL連携 Production Gate

- [ ] 本番で許可する外部システム名とドメインを責任者が確認した
- [ ] テスト参加者・商品・専用URLで、6段階の優先順位どおりに選択されることを確認した
- [ ] スマートフォンで投稿案表示、採用、コピー、使用履歴まで完了した
- [ ] URLが本文またはcaptionへ1回だけ入り、Prompt・台本・スライド本文へ混入しないことを確認した
- [ ] URL停止後に古い投稿案のコピーが拒否されることを確認した
- [ ] 別参加者・別GroupからURLと履歴を閲覧できないことを確認した
- [ ] CSVの正常行だけが下書き保存され、失敗行だけ理由付きで返ることを確認した
- [ ] LINE通知に完全URL、投稿本文、紹介Tokenが含まれないことを確認した
- [ ] クリック、成約、報酬、顧客情報を保存・表示していないことを確認した
- [ ] 対象commitへ`EXTERNAL_TRACKING_SMOKE`の確認証跡を記録した

詳細手順は`EXTERNAL_TRACKING_PRODUCTION_GATE.md`を正本とする。本確認がない状態では、外部成果URLを使う商品投稿を実利用者へ案内しない。
